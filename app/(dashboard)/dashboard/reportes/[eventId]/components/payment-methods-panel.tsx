...
export default function PaymentMethodsPanel({
  ...
  const maxCommissionCost =
    methodsWithCommission.length > 0
      ? Math.max(...methodsWithCommission.map((pm) => pm.commissionCostIfAll))
      : 0;
        ...
        <div className="rounded-lg border p-4 space-y-2 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-medium text-destructive">
              -
              {formatPrice(
                totalRevenue - baseProfit > 0 ? totalRevenue - baseProfit : 0,
              )}
            </span>
          </div>
          <Separator />
          ...
          {hasCommissions && hasTrackedOrders && (
            <>
              ...
              <div className="flex justify-between font-semibold">
                ...
                <span
                  className={netProfitActual >= 0 ? "" : "text-destructive"}
                >
              ...
              {hasUntrackedOrders && (
                <div className="text-xs text-muted-foreground italic pt-1">
                  ...
                  {formatPrice(revenueUntracked)} en ingresos) no están
        ...
        <div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {paymentMethods.map((pm) => {
              const hasCommission =
                pm.commissionPercentage !== null && pm.commissionPercentage > 0;
              const isNetPositive = pm.netProfitIfAll >= 0;
              const isActualPositive = baseProfit - pm.commissionActual >= 0;
                  ...
                  {pm.ordersTracked > 0 && (
                    <div className="text-sm space-y-1.5 border-t pt-3">
                      <div className="flex items-center gap-1 text-xs text-muted-foreground mb-1">
                        ...
                        <span>
                          Rastreado: {pm.ordersTracked} órdenes (
                          {formatPrice(pm.revenueTracked)})
                        </span>
                      </div>
                      {hasCommission && (
                        <>
